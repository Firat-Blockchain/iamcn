import { e as AbiSchema, bY as detectFeatures, cp as fetchAndCacheDeployMetadata, bZ as isFeatureEnabled, cH as getChainProvider, d5 as ContractWrapper, d6 as getAllPluginsAbi, bI as fetchContractMetadataFromAddress, co as fetchAndCachePublishedContractURI, d7 as getAllExtensionsAbi, d8 as caches, ce as getCreate2FactoryAddress, cl as computeDeploymentInfo, cm as encodeConstructorParamsForImplementation, cc as isContractDeployed, c5 as fetchSourceFilesFromMetadata, bP as extractConstructorParamsFromAbi, bT as resolveContractUriFromAddress, cx as getContractPublisherAddress, bX as fetchExtendedReleaseMetadata, c8 as COMMON_FACTORY, cd as isEIP155Enforced, d9 as CUSTOM_GAS_FOR_CHAIN, cn as getCreate2FactoryDeploymentInfo, bf as toWei, c9 as GAS_LIMIT_FOR_DEPLOYER, cb as DEPLOYER_ABI, ca as DEPLOYER_BYTECODE, cz as ChainId, cv as CONTRACT_ADDRESSES, da as AddressZero, cj as computeForwarderAddress, cU as PackInitializer, cQ as MarketplaceV3Initializer, cP as MarketplaceInitializer, cW as SplitInitializer, cZ as VoteInitializer, cY as TokenInitializer, cX as TokenDropInitializer, cO as EditionInitializer, cN as EditionDropInitializer, cR as MultiwrapInitializer, cV as SignatureDropInitializer, cS as NFTCollectionInitializer, cT as NFTDropInitializer, db as RPCConnectionHandler, cr as resolveAddress, b9 as normalizePriceValue, b7 as isNativeToken, bb as fetchCurrencyValue, dc as signTypedDataInternal, cC as NATIVE_TOKEN_ADDRESS, cy as getMultichainRegistryAddress, dd as buildTransactionFunction, aV as Transaction, c_ as PREBUILT_CONTRACTS_MAP, d2 as getContractName, G as CommonPlatformFeeSchema, z as CommonPrimarySaleSchema, H as CommonTrustedForwarderSchema, w as CommonContractSchema, y as CommonRoyaltySchema, I as CommonSymbolSchema, cJ as isChainConfig, cs as setSupportedChains, de as ContractPublisher, d1 as getContractTypeForRemoteName, cL as getSignerAndProvider, df as SmartContract, dg as getCompositePluginABI, dh as buildDeployTransactionFunction, cw as getContractAddressByChainId, di as getProcessEnv, dj as DropErc721ContractSchema, c$ as PREBUILT_CONTRACTS_APPURI_MAP, cG as EventType, aW as DeployTransaction } from './contract-publisher-a4c4b946.esm.js';
...
async function getMetadataForExtensions(publishedMetadataUri, storage) {
  ...
  const isExtensionRouter = isFeatureEnabled(AbiSchema.parse(compilerMetadata.abi), "ExtensionRouter");
  if (isExtensionRouter) {
    if (extendedMetadata && extendedMetadata.factoryDeploymentData?.implementationAddresses) {
      ...
      try {
        ...
        if (network && implementation) {
          const provider = getChainProvider(parseInt(network), {});
          const contract = new ContractWrapper(provider, implementation, getAllExtensionsAbi, {}, storage);
          const extensions = await contract.call("getAllExtensions");
          const extensionNames = Array.from(new Set(extensions.map(item => item.metadata.name)));
          const extensionUris = await Promise.all(extensionNames.map(name => {
            return fetchAndCachePublishedContractURI(name);
          }));
          extensionMetadata = (await Promise.all(extensionUris.map(async uri => {
            return fetchAndCacheDeployMetadata(uri, storage);
          }))).map(fetchedMetadata => fetchedMetadata.compilerMetadata);
        }
      } catch {}
...
async function getDeploymentInfo(metadataUri, storage, provider, create2Factory) {
  ...
  const extensionMetadata = await getMetadataForExtensions(metadataUri, storage);
