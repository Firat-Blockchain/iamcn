...
async function getMetadataForExtensions(publishedMetadataUri, storage) {
  ...
  const isExtensionRouter = contractPublisher.isFeatureEnabled(contractPublisher.AbiSchema.parse(compilerMetadata.abi), "ExtensionRouter");
  if (isExtensionRouter) {
    if (extendedMetadata && extendedMetadata.factoryDeploymentData?.implementationAddresses) {
      ...
      try {
        ...
        if (network && implementation) {
          const provider = contractPublisher.getChainProvider(parseInt(network), {});
          const contract = new contractPublisher.ContractWrapper(provider, implementation, contractPublisher.getAllExtensionsAbi, {}, storage);
